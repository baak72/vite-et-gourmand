...
const MenuDetailView = () => {
  ...
  // --- Récupération des données via Laravel ---
  useEffect(() => {
  ...
  if (!menu) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center pt-20 text-white">
        ...
        <button onClick={() => navigate('/menus')} className="text-amber-500 hover:text-amber-400 underline">
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white pt-36 pb-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <button onClick={() => navigate(-1)} className="flex items-center text-zinc-400 hover:text-amber-500 transition-colors group">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="relative group rounded-3xl overflow-hidden border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)] bg-zinc-900 aspect-4/3 lg:aspect-auto lg:h-[600px]">
            {menu.image ? (
              <img src={menu.image} alt={menu.nom_menu} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
            ) : (
          ...
          <div className="flex flex-col h-full justify-center space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-zinc-900/50 border border-white/5 p-6 rounded-2xl flex flex-col items-center text-center hover:border-amber-500/30 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* Valeur par défaut à 2 si non présente en base */}
                  <span className="text-3xl font-bold text-white">{menu.nombre_personne_minimum || 2}</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4">
              <button onClick={handleOrderClick} className="w-full group relative overflow-hidden bg-amber-500 text-black font-montserrat font-bold text-lg py-5 px-8 rounded-full shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_30px_rgba(245,158,11,0.4)] transition-all duration-300 transform hover:-translate-y-1">
                <span className="relative z-10 flex items-center justify-center gap-3">Démarrer la commande</span>
              ...
              {!user && (
                <p className="text-center text-zinc-500 text-xs mt-3">Vous devrez vous connecter pour finaliser la commande.</p>
              )}
            </div>
